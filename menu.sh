...
function instalarLibreriasEntornoVirtual()
{
     ...
     then
	     ...
	     pip install -r requirements.txt # para desinstalarlo hay que poner pip uninstall -r requirements.txt
     fi
     echo -e ""
     read -p "PULSA ENTER PARA CONTINUAR..."	
}
...
function crearServicioSystemdFlask()
{
    aux=$(ls -lias  /etc/systemd/system | grep flask.service)
   	 ...
   	 then
         ...
         sudo systemctl status flask  ## no estoy segura que se active con source... pero descarto que sea con sudo
}
...
###########################################################
#                  19) VISUALIZAR ERRORES NGINX          # sudo lsof -i :5000 Mira que procesos están escuchando en ese puerto
###########################################################
function verNginxLogs()
{
	...
	# echo ""
        # read -p "PULSA ENTER PARA CONTINUAR..."
        diaAct=$(date +%d) #saca el día numerico
	mesAct=$(date +%b) #saca el mes completo
	dia=11
	mes="may"
	if expr $diaAct = $dia
	then 
		echo -e "Es el día de hoy"
		if test $mesAct = $mes
		then
			echo -e "Es el mes actual"
		fi
	fi
}
...
function controlarIntentosConexionSSH()
{
	...
	less auth.log.txt | tr -s ' ' '@' > out.txt  #no sé si hay que meterle un less delante como pone la pizarra
	#Sustituye los espacios ' ' por '@'
	#echo -e "Lo convierto a texto porque salen errores en la transformación"
	#iconv -f binary -t utf-8 /home/carmela/Descargas/pruebasC/out.txt > /home/carmela/Descargas/pruebasC/out_utf8.txt
	...
	do   ## para que no coja las de Connection  
		#echo $linea
		...
		dia=$(echo $linea | cut -d '@' -f 2)  #cut -d es un delimitador, delimita las líneas con @ y coge la primera columna 'f
		mes=$(echo $linea | cut -d '@' -f 1)
		hora=$(echo $linea | cut -d '@' -f 3)
		aux2=$(echo $linea | grep "repeated") 
		if [ -z $aux2 ]
		then
		...
		#aux3=$(echo $linea | grep "May") Comprobaría si el mes es mayo
		#aux4=$(echo $linea | grep "@9@") Comprobaría si el día es el 9
		
		#if [ -z "$aux3" ]
		#then 
		#	echo -e "Mayo no es el mes"
		#else 
		#	echo -e "Mayo es el mes"
		#fi
	        #if [ -z "$aux4" ]
		#then 
		#	echo -e "No es el día 9"
		#else 
		#	echo -e "Es el día 9"
		#fi
...
function mostrar_log_ssh()
{
	...
	for linea in $(grep "sshd" out.txt  | grep -E "Accepted") 
	...
	do   ## para que no coja las de Connection  
		...
		then 
			if test "May" = $mes
			then
				echo -e "Account name: $user ip: $ip  time: $hora"
			fi
		fi
	done
	read -p "PULSA ENTER PARA CONTINUAR..."
...
# {
	# aux=$(sudo systemctl status nginx | grep "Active: active")  # Puede pedir mirar si el flask.service está activado
	# if [ -z "$aux" ]
	# then
		# echo -e "El demonio nginx no está activo"
		# echo -e "No hace falta pararlo"
	# else 
		# echo -e "El demonio nginx sigue activo"
		# echo -e "Vamos a parar nginx..."
		# sudo systemctl stop nginx
		# echo -e "Comprueba que nginx no está activo..."
		# sudo systemctl status nginx
	# fi
	# aux1=$(aptitude show nginx | grep "State: installed") # Para gunicorn hay que mirar si el gunicorn está instalado, si lo está eliminar con sudo pip uninstall gunicorn y en la carpeta 
	# aux2=$(aptitude show nginx | grep "Estado: instalado") # hay que mirar si está gunicorn en la carpeta /var/www/formulario/produccion/venv/bin/ y eliminar el archivo
	# aux3=$aux$aux1
	# if [ -z "$aux3" ]
	# then 
		# echo -e "Nginx no está instalado"
		# echo -e "No hace falta desinstalarlo"
	# else
		# echo -e "Nginx está instalado"
		# echo -e "Vamos a desinstalar nginx..."
...
### Main ###
opcionmenuppal=0
while test $opcionmenuppal -ne 22 #23
do
	#Muestra el menu
        ...
        echo -e "13 Configuración de Gunicorn: Permisos a www-data \n"
        ...
        echo -e "22 Mostrar información de las conexiones exitosas a un ordenador remoto \n"
	echo -e "23 Salir del menu \n"
	# echo -e "23 Desinstalar nginx \n"
